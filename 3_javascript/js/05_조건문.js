...
function check4() {
    ...
    // 입력된 나이의 길이가 0인 경우 == 입력된 값이 없을때 (미입력)
    if (inputAge.value.length == 0) {
        ...
        } else if (age <= 19) { // 청소년 14~19까지 (0~13까지는 위에서 걸러졌음)
            alert("청소년");
...
function calc(op) {
    ...
    switch (op) {
        ...
        // 마지막이라 (break)생략가능, 필요없을땐 디폴트도 없어도됨
