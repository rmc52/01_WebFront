...
function print1(num, str) { // 함수선언
    ...
    console.log(`${num} 은/는 ${str} 입니다`); // 함수 정의
...
btn1.addEventListener("click", function () {
    ...
    if (Number(value) == 0) result = "0"; // 한줄은 중괄호 생략가능
    else if (Number(value) > 0) result = "양수";
    else result = "음수";


    // 함수 호출(값 전달)
    // 중요!!!! 전달인자의 값의 순서가 매우 중요
    // 컨트롤누른상태로 함수명클릭 = 찾아감
...
function print2(fn) {
    // console.log(fn);
    console.log(` a + b = ${fn(3,4)}`); // function(a, b) 함수의 호출이 fn(3,4)다
...
document.querySelector("#btn2c").addEventListener("click", function() {
    ...
    const sumFn = function(a, b) {
       return a + b; // 함수를 종료함과 동시에 
                    // a + b 를 수행한 값을 가지고 호출한 곳으로 되돌아감
    }
...
function returnTest(num1, num2) { 
    return num1 * num2; // 함수 종료 + return 옆에 작성된 값을 가지고 호출한 쪽으로 돌아감
